...
**Next steps toward the MVP** are the vertical slices in *Phase 7 → Execution plan (approved build order)* below, which cover functional cases FT-01 through FT-14. Steps 0–4 (Argon2id demo credentials, persistence layer, auth slice, picking happy path, picking negative/concurrency/idempotency) are complete and evidenced (`docs/evidence/2026-07-13-phase7-step0-argon2id.md` through `…-step4-picking-negative-path.md`); Step 5 (admin endpoints — order creation with atomic multi-bin allocation, block/resume recovery, order/task reads, stock adjustments) is the next slice. The MVP as a whole is defined by FT-01 through FT-19 in `docs/functional-test-specification.md`: Phase 7 delivers the API cases, Phase 8 the label/dashboard cases (FT-17, FT-18), Phase 9 the configuration/observability cases and retained evidence (FT-15, FT-16), and Phase 10 plus a scope review close FT-19. Artifacts belonging to phases not yet implemented and evidenced remain **provisional drafts**.
