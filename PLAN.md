...
**Next steps toward the MVP** are the vertical slices in *Phase 7 → Execution plan (approved build order)* below, which cover functional cases FT-01 through FT-14. Steps 0–3 (Argon2id demo credentials, persistence layer, auth slice, picking happy path) are complete and evidenced (`docs/evidence/2026-07-13-phase7-step0-argon2id.md` through `…-step3-picking-happy-path.md`); Step 4 (picking negative path / concurrency / idempotency — wrong scans, quantity mismatch, concurrent claim race, one-active-task-per-user/device, confirmation-UUID reuse with a different payload) is the next slice. The MVP as a whole is defined by FT-01 through FT-19 in `docs/functional-test-specification.md`: Phase 7 delivers the API cases, Phase 8 the label/dashboard cases (FT-17, FT-18), Phase 9 the configuration/observability cases and retained evidence (FT-15, FT-16), and Phase 10 plus a scope review close FT-19. Artifacts belonging to phases not yet implemented and evidenced remain **provisional drafts**.
