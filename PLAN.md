...
**Next steps toward the MVP** are the vertical slices in *Phase 7 → Execution plan (approved build order)* below, which cover functional cases FT-01 through FT-14. Step 0 (Argon2id demo credentials), Step 1 (persistence layer), and Step 2 (auth slice) are complete and evidenced (`docs/evidence/2026-07-13-phase7-step0-argon2id.md`, `…-step1-persistence.md`, `…-step2-auth.md`); Step 3 (picking happy path — `GET /hht/tasks/next` atomic claim, scan-location, scan-article, and the confirm transaction) is the next slice. The MVP as a whole is defined by FT-01 through FT-19 in `docs/functional-test-specification.md`: Phase 7 delivers the API cases, Phase 8 the label/dashboard cases (FT-17, FT-18), Phase 9 the configuration/observability cases and retained evidence (FT-15, FT-16), and Phase 10 plus a scope review close FT-19. Artifacts belonging to phases not yet implemented and evidenced remain **provisional drafts**.
