...
```bash
export WMS_MFC_ADAPTER=telegram
export WMS_MFC_TELEGRAM_BASE_URL="http://<wcs-host>:<port>"
export WMS_MFC_TRANSPORT_SOURCE_LOCATION="<existing-location-code>"
export WMS_MFC_TRANSPORT_DESTINATION_LOCATION="<existing-location-code>"
...
- **Network:** telegram delivery is an *outbound* HTTP POST from the WMS to
  ...
  source range rather than opening a new port.
